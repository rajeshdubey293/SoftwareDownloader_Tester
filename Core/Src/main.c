...
uint32_t calCulateCRC(uint8_t*  ptr, uint32_t);
	...
	while(1)
	{
		...
		if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_SET)
		{
			...
			if(uartDataRx[1] == 0xFFFFFFFF)
			{
				...
				uint32_t crc_Value = calCulateCRC(dataArray, payloadLen);
			...
			{
				printmsg("Unknown Tester\r\n");
				HAL_Delay(500);
				printmsg("Existing from Programming Mode\r\n");
				HAL_Delay(500);
			}
		}
	}
...
uint32_t lenOfHexFile(void)
{
	uint32_t *tempValue = (uint32_t*)dataArray;
	uint32_t len = 0;
...
uint32_t calCulateCRC(uint8_t *pData, uint32_t len)
{
